...
NMarkers, FrameRate, Markers_Names, NFrames, Time, Sets, Anth, Markers = csv2strc('Ejemplo_Pareja')

# Listas dado que hay 2 personas
MMarkers = []
MCenters = []
...
for i in range(len(Sets)):
    ...
    # _______________________Cinematica _______________________
    ...
    # Llenar listas dado que hay dos personas
    PelvisCG.append(Pelviscg)
    R_ThighCG.append(R_Thighcg)
    L_ThighCG.append(L_Thighcg)
    R_CalfCG.append(R_Calfcg)
    L_CalfCG.append(L_Calfcg)
    R_FootCG.append(R_Footcg)
    L_FootCG.append(L_Footcg)

    Pelvis_segi.append(Pelvis_i)
    Pelvis_segj.append(Pelvis_j)
    Pelvis_segk.append(Pelvis_k)

    R_Thigh_segi.append(R_Thigh_i)
    R_Thigh_segj.append(R_Thigh_j)
    R_Thigh_segk.append(R_Thigh_k)

    L_Thigh_segi.append(L_Thigh_i)
    L_Thigh_segj.append(L_Thigh_j)
    L_Thigh_segk.append(L_Thigh_k)

    R_Calf_segi.append(R_Calf_i)
    R_Calf_segj.append(R_Calf_j)
    R_Calf_segk.append(R_Calf_k) 

    L_Calf_segi.append(L_Calf_i)
    L_Calf_segj.append(L_Calf_j)
    L_Calf_segk.append(L_Calf_k)

    R_Foot_segi.append(R_Foot_i)
    R_Foot_segj.append(R_Foot_j)
    R_Foot_segk.append(R_Foot_k)

    L_Foot_segi.append(L_Foot_i)
    L_Foot_segj.append(L_Foot_j)
    L_Foot_segk.append(L_Foot_k)
    ...
    if i == 1:
        ...
        Xmin = min(np.min(MMarkers[0][:,:,0]),np.min(MCenters[0][:,:,0]),np.min(MMarkers[1][:,:,0]),np.min(MCenters[1][:,:,0]))
        Xmax = max(np.max(MMarkers[0][:,:,0]),np.max(MCenters[0][:,:,0]),np.max(MMarkers[1][:,:,0]),np.max(MCenters[1][:,:,0]))

        Ymin = min(np.min(MMarkers[0][:,:,1]),np.min(MCenters[0][:,:,1]),np.min(MMarkers[1][:,:,1]),np.min(MCenters[1][:,:,1]))
        Ymax = max(np.max(MMarkers[0][:,:,1]),np.max(MCenters[0][:,:,1]),np.max(MMarkers[1][:,:,1]),np.max(MCenters[1][:,:,1]))

        Zmin = min(np.min(MMarkers[0][:,:,2]),np.min(MCenters[0][:,:,2]),np.min(MMarkers[1][:,:,2]),np.min(MCenters[1][:,:,2]))
        Zmax = max(np.max(MMarkers[0][:,:,2]),np.max(MCenters[0][:,:,2]),np.max(MMarkers[1][:,:,2]),np.max(MCenters[1][:,:,2]))+tamseg
        ...
        for a in range(0, NFrames, int(velocidad)):
            ...
            for i2 in range(len(Sets)):
                # ---------------- Marcadores ----------------
               ax.scatter(MMarkers[i2][a,:,0],MMarkers[i2][a,:,1],MMarkers[i2][a,:,2], color = 'b', marker = 'o', s = 8)
               # ---------------- Centros ----------------
               ax.plot(MCenters[i2][a,:,0],MCenters[i2][a,:,1],MCenters[i2][a,:,2], color = 'k', linestyle='-', marker = '*', markersize = 5)
               # ------------ Vectores unitarios ------------
               frameref(ax,Pelvis_segi[i2][a],Pelvis_segj[i2][a],Pelvis_segk[i2][a],PelvisCG[i2][a],'c',2,segmentos)
               frameref(ax,R_Thigh_segi[i2][a],R_Thigh_segj[i2][a],R_Thigh_segk[i2][a],R_ThighCG[i2][a],'g',2,segmentos)
               frameref(ax,L_Thigh_segi[i2][a],L_Thigh_segj[i2][a],L_Thigh_segk[i2][a],L_ThighCG[i2][a],'r',2,segmentos)
               frameref(ax,R_Calf_segi[i2][a],R_Calf_segj[i2][a],R_Calf_segk[i2][a],R_CalfCG[i2][a],'g',2,segmentos)
               frameref(ax,L_Calf_segi[i2][a],L_Calf_segj[i2][a],L_Calf_segk[i2][a],L_CalfCG[i2][a],'r',2,segmentos)
               frameref(ax,R_Foot_segi[i2][a],R_Foot_segj[i2][a],R_Foot_segk[i2][a],R_FootCG[i2][a],'g',2,segmentos)
               frameref(ax,L_Foot_segi[i2][a],L_Foot_segj[i2][a],L_Foot_segk[i2][a],L_FootCG[i2][a],'r',2,segmentos)
        ...
        # Calcular el tamaño máximo de los ejes en cada dimensión
        max_range = max(xlim[1]-xlim[0], ylim[1]-ylim[0], zlim[1]-zlim[0])

        # Centrar los límites de los ejes y establecer el mismo tamaño en todas las dimensiones
        ...
        ax.set_ylabel('Eje Y')
        ax.set_zlabel('Eje Z')
        ax.set_title(str(Sets[0])+'&'+str(Sets[1]),size = 25)
